...
- `shaftExchange` captures particles horizontally toward the nearest of three street shafts and vents them upward. Shaft lift begins in the intake band at `y = 0.4`, and the position opening matches the capture column so particles are not roof-clamped before entering. `shaftFans` gates visible powered fan hardware, while `shaftFanVelocity` adds controllable upward shaft velocity. `downFans` remains a separate station/ceiling force; `floorAirMovers` drives visible low fans across the platform floor toward the stair route; `ceilingFans` sweeps the ceiling band toward the egress; `grooves` adds low-energy passive flow; and `stackEffect` modulates thermal lift and heat removal through the shafts.
...
- `clerestoryOpen` ranges from `0` (closed) to `1` (fully open). Two window rows sit on the horizontal gap edges and span the complete vertical gap, rotating outward as the opening increases.

...
- `CameraController` is the sole owner of `OrbitControls`. Add fixed perspectives through `CAMERA_VIEWS`; fixed modes animate to their destination and `orbital` enables auto-rotation. OrbitControls `start` must clear the active mode so manual rotate, pan, and zoom are never overwritten and no preset remains selected.
...
- The fluid domain is `x = [-16, 34]`, `y = [-6.2, 14]`, and `z = [-5, 5]`. X wraps across the full route; the lower bound includes the flood gallery and the upper bound includes the stair exit, shaft outlets, and street volume.
...
- Surface crosswind must create upward pressure draw at each shaft outlet and return below the fluid ceiling. Preserve both terms together so particles form outlet plumes without collecting at `FLUID_BOUNDS.maxY`.

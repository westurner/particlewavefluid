...
- **waveinterferencesim:** an animated field for composing one or more waves with independent wavelength, amplitude, phase offset, phase rate, decay rate, signal origin, signal direction, XYZ signal rotation, polarization, and standard, circular, or helical phase modes. Wave phase is evaluated in an origin-relative orthonormal frame aligned to each direction vector, then rotated as a rigid directed basis. Helical modes use an integer unit topological charge around that axis, so they model mathematically consistent helical phase fronts rather than the previous global-angle approximation. Decay applies the downstream envelope $e^{-\alpha s}$, where $s$ is distance along the rotated propagation axis. Scalar mode preserves the original height-field visualization; transverse and longitudinal modes render displacement vectors, with longitudinal displacement parallel to propagation. Electromagnetic mode adds a transverse electric field and derives the magnetic field as $\mathbf B=\hat{\mathbf k}\times\mathbf E$ in normalized units. EM-Tensor-Gaussian mode derives a normalized Maxwell-stress-like tensor from those fields, applies a transverse Gaussian beam waist, and uses its normalized weight to size and fade particle splatters. These are local normalized field visualizations, not a full finite-aperture Maxwell solver. A longitudinal mode is useful for mechanical or acoustic-style waves, but it is not a source-free electromagnetic plane wave, whose electric field is transverse. Active signal sources render direction arrows, and the SimpleAttractor-style damped orbit controls can be enabled or disabled. Signal source presets provide a continuous-wave laser and a continuous-wave laser w/ Helical-L and Helical-R. Independent occlusion-map presets provide an open field, pinhole, single slit, double slit, Sierpinski carpet, unilluminable room, and boulder shadow transmission masks. Constructive interference and signed superposition are independent response layers that can be enabled separately or together. Visualization controls include particle count, size, opacity, shape, vector derivative order, and double-sided rendering. Each wave can be duplicated or removed with confirmation. The Wave states library teaches single traveling waves, cancellation, constructive addition, standing waves, quadrature, circular and counter-rotating waves, helical pairs, and a mixed field. Users can save named states locally; each saved state retains all eight wave slots, the active slot count, and the interference layer toggles.
...
- The expandable Vent flow charts section gives the west, central, and east shafts independent signed speed controls from reverse (`-1`) through off (`0`) to forward (`+1`). Each chart samples the live GPU particle field at the shaft intake and road-level outlet, displaying signed vertical velocity and particle temperature histories; positive flow is upward and negative flow is downward.

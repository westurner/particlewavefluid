...
function cameraFrameOffset(camera, cameraPosition, target, viewport, parametersVisible) {
  if (!parametersVisible || viewport.width <= PARAMETERS_PANEL_LAYOUT.breakpoint) return new Vector3();
...
function ParticleField({ settings, trainRef, onTelemetry, onGpuError }) {
  ...
  const particleGeometry = useMemo(() => {
    ...
    geometry.setAttribute('aSimulationUv', new InstancedBufferAttribute(createSimulationUvs(simulationResolution, particleCount), 2));
    return geometry;
    ...
    try {
      ...
      const simulation = createGpuParticleField({
        ...
        initialize: ({ particleIndex, positionData, velocityData, offset }) => {
          ...
          positionData[offset] = MathUtils.lerp(FLUID_BOUNDS.minX, FLUID_BOUNDS.maxX, Math.random());
          positionData[offset + 1] = isSurfaceParticle
            ? surfaceParticleSeedY(particleIndex, particleCount, initialVerticalLayout.streetY)
            : MathUtils.lerp(PARTICLE_SEED_BOUNDS.minY, PARTICLE_SEED_BOUNDS.maxY, Math.random());
          positionData[offset + 2] = MathUtils.lerp(FLUID_BOUNDS.minZ, FLUID_BOUNDS.maxZ, Math.random());
      ...
      positionVariable.material.uniforms.uRoofPitch = { value: MathUtils.degToRad(INITIALS.roofPitch) };
      ...
      velocityVariable.material.uniforms.uShaftControls = { value: new Vector3(...INITIALS.shaftControls) };
      ...
      velocityVariable.material.uniforms.uRoofPitch = { value: MathUtils.degToRad(INITIALS.roofPitch) };
  ...
  useFrame((state, delta) => {
    ...
    positionVariable.material.uniforms.uRoofPitch.value = MathUtils.degToRad(currentSettings.roofPitch);
    ...
    velocityVariable.material.uniforms.uRoofPitch.value = MathUtils.degToRad(currentSettings.roofPitch);
...
function VentilationInfrastructure({ settings }) {
  ...
  const roofPitchRadians = MathUtils.degToRad(settings.roofPitch);
...
function createUnderfillGeometry(sections, baseY) {
  ...
  shape.closePath();
  const geometry = new ExtrudeGeometry(shape, { depth: STAIR_ROUTE.width, bevelEnabled: false });
  ...
  return (
    <group>
      ...
      <mesh geometry={outerGroundGeometry} receiveShadow>
        <meshStandardMaterial color="#596763" transparent opacity={0.09} depthWrite={false} side={DoubleSide} roughness={0.95} />
      </mesh>
    </group>
  );
}
    ...
    <group>
      {insulationSegments.map((segment) => (
        ...
        >
          ...
          <meshStandardMaterial color="#596763" transparent opacity={0.07} depthWrite={false} side={DoubleSide} roughness={0.95} />
        </mesh>
      ))}
  ...
  return (
    <group visible={enabled}>
      <mesh position={[galleryCenterX, FLOOD_GALLERY.y, FLOOD_GALLERY.z]} rotation={[0, 0, Math.PI / 2]}>
        ...
        <meshStandardMaterial color="#15383e" side={BackSide} roughness={0.9} metalness={0.1} transparent opacity={0.82} />
      </mesh>
  ...
  return (
    <group>
      ...
      <mesh position={[trackCenterX, tunnelCenterY, TRACK_ROUTE.centerZ]}>
        ...
        <meshStandardMaterial color="#8fb8b5" transparent opacity={0.035} depthWrite={false} side={DoubleSide} roughness={0.2} metalness={0.08} />
      </mesh>
